...
CREATE TABLE `stats_batter` (
  ...
  `ing11` varchar(10) DEFAULT NULL,
  `ing12` varchar(10) DEFAULT NULL,
...
CREATE TABLE `stats_scoreboard` (
  ...
  `ing11` varchar(3) DEFAULT NULL,
  `ing12` varchar(3) DEFAULT NULL,
...
FROM `live_body` `lb`
LEFT JOIN `live_header` `lh` 
    ON  `lb`.`game_info_id` = `lh`.`game_info_id` 
    AND `lb`.`scene`        = `lh`.`scene`
LEFT JOIN `game_info` `gi` 
    ON  `lb`.`game_info_id` = `gi`.`id`
LEFT JOIN `team_info` `ti` 
    ON  `lb`.`game_info_id`   = `ti`.`game_info_id` 
    AND `lb`.`scene`          = `ti`.`scene` 
    AND `ti`.`team_initial_kana` = `lh`.`b_team`
LEFT JOIN `game_order` `go` 
    ON  `ti`.`id`   = `go`.`team_info_id` 
