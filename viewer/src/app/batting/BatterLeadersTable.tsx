...
import { BatterLeaderRow } from "./types";

interface Props {
  rows: BatterLeaderRow[];
}

export function BatterLeadersTable({ rows }: Props) {
  return (
      ...
      <table className="min-w-full text-xs text-left">
        <thead className="bg-slate-50 text-slate-600 font-semibold border-b border-slate-200">
          <tr>
            ...
            <th className="p-2.5 text-right font-semibold text-blue-600">打点</th>
            <th className="p-2.5 text-right">四球</th>
            <th className="p-2.5 text-right">三振</th>
            <th className="p-2.5 text-right font-bold text-slate-900 bg-slate-50/50">打率</th>
            <th className="p-2.5 text-right font-bold text-slate-800">出塁率</th>
          </tr>
        </thead>
